...
function init(app, io, mountPath = '') {
    ...
    roomModeration.registerRoutes({
        ...
        mountPath,
        games,
        snapshotGame: game => sanitizeGameForPlayer(game),
        emitGameUpdate: (gameId, game) => gameIo.to(gameId).emit('update', sanitizeGameForPlayer(game)),
        endToyControl
    });
    ...
    app.post(`${mountPath}/api/join`, (req, res) => {
        ...
        if (!game.player1 && role !== '2') {
            game.player1 = { uuid, name, ready: false, ships: [], connected: false, toyEnabled: false, qrCode: null, linkCode: null, qrError: null };
            assignedRole = '1';
        } else if (!game.player2 && role !== '1') {
            game.player2 = { uuid, name, ready: false, ships: [], connected: false, toyEnabled: false, qrCode: null, linkCode: null, qrError: null };
            assignedRole = '2';
        }
    ...
    app.post(`${mountPath}/api/fire`, (req, res) => {
        ...
        if (allShipsSunk(opponentShips, opponentReceivedShots)) {
            ...
            // Lovense post-game control setup
            const player1 = game.player1;
            const player2 = game.player2;

            if (player1 && player2) {
                const winnerPlayer = playerNum === 1 ? player1 : player2;
                const loserPlayer = playerNum === 1 ? player2 : player1;

                const hasHumanOpponent = !!(loserPlayer && loserPlayer.uuid && !loserPlayer.uuid.startsWith('cpu-'));
                const opponentHasToy = isToyActive(loserPlayer);
                const selfHasToy = isToyActive(winnerPlayer);
